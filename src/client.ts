...
export const telegramClient = () => {
  return {
    ...
    getActions: (
      ...
      // Custom actions call fetch directly, bypassing Better Auth's dynamic proxy.
      const $fetch = async <T = unknown>(
        path: string,
        options?: FetchOptions
      ) => {
        const result = await baseFetch<T>(path, options);
        if (
          result &&
          !result.error &&
          ([
            ...
            (path === "/sign-in/social" && options?.body?.idToken))
        )
