...
let _better_auth_core_utils_error_codes = require("@better-auth/core/utils/error-codes");
let _better_auth_core_context = require("@better-auth/core/context");
let better_auth_cookies = require("better-auth/cookies");
let better_auth_db = require("better-auth/db");
let _better_auth_core_oauth2 = require("@better-auth/core/oauth2");
...
function createConfigEndpoint(config) {
	return { getTelegramConfig: (0, better_auth_api.createAuthEndpoint)("/telegram/config", { method: "GET" }, async (ctx) => ctx.json({
...
const ERROR_CODES = (0, _better_auth_core_utils_error_codes.defineErrorCodes)({
...
async function validateTelegramUser(ctx, user, action, method) {
	...
	try {
		...
		throw new better_auth_api.APIError("FORBIDDEN", {
			code: "validation_failed",
			message: "User validation failed"
		});
	}
	if (result?.error) throw new better_auth_api.APIError("FORBIDDEN", {
		code: result.error,
		message: result.errorDescription || result.error
	});
}
const telegramSessionMiddleware = (0, better_auth_api.createAuthMiddleware)(async (ctx) => {
	ctx.context.session = null;
	const session = await (0, better_auth_api.getSessionFromCtx)(ctx, { disableCookieCache: true });
	if (!session) throw better_auth_api.APIError.from("UNAUTHORIZED", ERROR_CODES.NOT_AUTHENTICATED);
	const age = ctx.context.sessionConfig.freshAge;
	if (age !== 0 && Date.now() - new Date(session.session.createdAt).getTime() >= age * 1e3) throw new better_auth_api.APIError("FORBIDDEN", {
...
async function signInTelegram(ctx, identity, mapped, allowSignup, method) {
	...
	const user = await (0, _better_auth_core_context.runWithTransaction)(ctx.context.adapter, async () => {
		const account = await internal.findAccountByKey(key);
		if (account) {
			const existing = await internal.findUserById(account.userId);
			if (!existing) throw better_auth_api.APIError.from("UNAUTHORIZED", ERROR_CODES.INVALID_AUTHENTICATION);
			await validateTelegramUser(ctx, existing, "sign-in", method);
			return existing;
		}
		if (!allowSignup) throw better_auth_api.APIError.from("FORBIDDEN", ERROR_CODES.USER_CREATION_DISABLED);
		if (await (await (0, _better_auth_core_context.getCurrentAdapter)(ctx.context.adapter)).findOne({
			model: "user",
			where: [{
				field: "telegramId",
				value: String(identity.id)
			}]
		})) throw new better_auth_api.APIError("CONFLICT", {
		...
		if (!created) throw better_auth_api.APIError.from("FORBIDDEN", ERROR_CODES.USER_CREATION_DISABLED);
		...
		if (!linked || linked.userId !== created.id || linked.accountId !== key.accountId || linked.providerId !== "telegram") throw better_auth_api.APIError.from("FORBIDDEN", ERROR_CODES.INVALID_AUTHENTICATION);
		return created;
	});
	const session = await internal.createSession(user.id);
	if (!session || session.userId !== user.id) throw better_auth_api.APIError.from("FORBIDDEN", ERROR_CODES.INVALID_AUTHENTICATION);
	await (0, better_auth_cookies.setSessionCookie)(ctx, {
		session,
		user
	});
	return ctx.json({
		user: (0, better_auth_db.parseUserOutput)(ctx.context.options, user),
		session: (0, better_auth_db.parseSessionOutput)(ctx.context.options, session)
	});
}
//#endregion
...
function createMiniAppEndpoints(config) {
	return {
		signInWithMiniApp: (0, better_auth_api.createAuthEndpoint)("/telegram/miniapp/signin", {
			method: "POST",
			use: [better_auth_api.formCsrfMiddleware]
		}, async (ctx) => {
			const initData = (await ctx.body)?.initData;
			if (!initData || typeof initData !== "string") throw better_auth_api.APIError.from("BAD_REQUEST", ERROR_CODES.INIT_DATA_REQUIRED);
			if (!config.botToken) throw better_auth_api.APIError.from("INTERNAL_SERVER_ERROR", ERROR_CODES.BOT_TOKEN_REQUIRED);
			if (!await verifyMiniAppInitData(initData, config.botToken, config.maxAuthAge)) throw better_auth_api.APIError.from("UNAUTHORIZED", ERROR_CODES.INVALID_MINI_APP_INIT_DATA);
			const data = parseMiniAppInitData(initData);
			if (!validateMiniAppData(data)) throw better_auth_api.APIError.from("BAD_REQUEST", ERROR_CODES.INVALID_MINI_APP_DATA_STRUCTURE);
			if (!data.user) throw better_auth_api.APIError.from("BAD_REQUEST", ERROR_CODES.NO_USER_IN_INIT_DATA);
		...
		validateMiniApp: (0, better_auth_api.createAuthEndpoint)("/telegram/miniapp/validate", { method: "POST" }, async (ctx) => {
			const initData = (await ctx.body)?.initData;
			if (!initData || typeof initData !== "string") throw better_auth_api.APIError.from("BAD_REQUEST", ERROR_CODES.INIT_DATA_REQUIRED);
			if (!config.botToken) throw better_auth_api.APIError.from("INTERNAL_SERVER_ERROR", ERROR_CODES.BOT_TOKEN_REQUIRED);
...
function createTelegramOIDCProvider(botToken, options = {}) {
	...
	const jwks = (0, jose.createRemoteJWKSet)(new URL(TELEGRAM_OIDC_JWKS_URI), {
	...
	const verifyToken = async (token, nonce) => {
		try {
			const { kid, alg } = (0, jose.decodeProtectedHeader)(token);
	...
	return {
		...
		createAuthorizationURL({ state, codeVerifier, scopes, redirectURI, idTokenNonce, loginHint, additionalParams }) {
			...
			return (0, _better_auth_core_oauth2.createAuthorizationURL)({
				id: TELEGRAM_OIDC_PROVIDER_ID,
...
function createWidgetEndpoints(config) {
	return {
		signInWithTelegram: (0, better_auth_api.createAuthEndpoint)("/telegram/signin", {
			method: "POST",
			use: [better_auth_api.formCsrfMiddleware]
		}, async (ctx) => {
			if (!config.botToken) throw better_auth_api.APIError.from("INTERNAL_SERVER_ERROR", ERROR_CODES.BOT_TOKEN_REQUIRED);
			const body = await ctx.body;
			if (!validateTelegramAuthData(body)) throw better_auth_api.APIError.from("BAD_REQUEST", ERROR_CODES.INVALID_AUTH_DATA);
			const telegramData = body;
			if (!await verifyTelegramAuth(telegramData, config.botToken, config.maxAuthAge)) throw better_auth_api.APIError.from("UNAUTHORIZED", ERROR_CODES.INVALID_AUTHENTICATION);
			const defaultUserData = {
		...
		linkTelegram: (0, better_auth_api.createAuthEndpoint)("/telegram/link", {
			method: "POST",
			use: [telegramSessionMiddleware]
		}, async (ctx) => {
			if (!config.allowUserToLink || ctx.context.options.account?.accountLinking?.enabled === false) throw better_auth_api.APIError.from("FORBIDDEN", ERROR_CODES.LINKING_DISABLED);
			const body = await ctx.body;
			const session = ctx.context.session;
			if (!session?.user?.id) throw better_auth_api.APIError.from("UNAUTHORIZED", ERROR_CODES.NOT_AUTHENTICATED);
			if (!config.botToken) throw better_auth_api.APIError.from("INTERNAL_SERVER_ERROR", ERROR_CODES.BOT_TOKEN_REQUIRED);
			if (!validateTelegramAuthData(body)) throw better_auth_api.APIError.from("BAD_REQUEST", ERROR_CODES.INVALID_AUTH_DATA);
			const telegramData = body;
			if (!await verifyTelegramAuth(telegramData, config.botToken, config.maxAuthAge)) throw better_auth_api.APIError.from("UNAUTHORIZED", ERROR_CODES.INVALID_AUTHENTICATION);
			...
			if (existingAccount && existingAccount.userId !== session.user.id) throw better_auth_api.APIError.from("CONFLICT", ERROR_CODES.TELEGRAM_ALREADY_LINKED_OTHER);
			if (existingAccount) throw better_auth_api.APIError.from("CONFLICT", ERROR_CODES.TELEGRAM_ALREADY_LINKED_SELF);
			if ((await ctx.context.internalAdapter.findAccounts(session.user.id)).some((account) => account.providerId === "telegram")) throw better_auth_api.APIError.from("CONFLICT", ERROR_CODES.TELEGRAM_ALREADY_LINKED_SELF);
			...
			const user = await (0, _better_auth_core_context.runWithTransaction)(ctx.context.adapter, async () => {
				...
				if (!account || account.userId !== session.user.id || account.accountId !== String(telegramData.id)) throw better_auth_api.APIError.from("FORBIDDEN", ERROR_CODES.INVALID_AUTHENTICATION);
				const updated = await ctx.context.internalAdapter.updateUser(session.user.id, {
					telegramId: String(telegramData.id),
					telegramUsername: telegramData.username
				});
				if (!updated) throw better_auth_api.APIError.from("FORBIDDEN", ERROR_CODES.INVALID_AUTHENTICATION);
				return updated;
			});
			await (0, better_auth_cookies.setSessionCookie)(ctx, {
		...
		unlinkTelegram: (0, better_auth_api.createAuthEndpoint)("/telegram/unlink", {
			method: "POST",
			use: [telegramSessionMiddleware]
		}, async (ctx) => {
			const session = ctx.context.session;
			...
			if (!account) throw better_auth_api.APIError.from("NOT_FOUND", ERROR_CODES.NOT_LINKED);
			if ((await ctx.context.internalAdapter.findAccounts(session.user.id)).length <= 1 && !ctx.context.options.account?.accountLinking?.allowUnlinkingAll) throw new better_auth_api.APIError("BAD_REQUEST", {
			...
			const user = await (0, _better_auth_core_context.runWithTransaction)(ctx.context.adapter, async () => {
				...
				if ((await ctx.context.internalAdapter.findAccounts(session.user.id)).some((item) => item.id === account.id)) throw better_auth_api.APIError.from("FORBIDDEN", ERROR_CODES.INVALID_AUTHENTICATION);
				const updated = await ctx.context.internalAdapter.updateUser(session.user.id, {
					telegramId: null,
...
//#endregion
exports.createTelegramOIDCProvider = createTelegramOIDCProvider;
