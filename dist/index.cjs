...
__export(src_exports, {
  createTelegramOIDCProvider: () => createTelegramOIDCProvider,
  telegram: () => telegram
});
...
function createConfigEndpoint(config) {
  return {
    getTelegramConfig: (0, import_api.createAuthEndpoint)(
      "/telegram/config",
      {
        method: "GET"
      },
...
// src/constants.ts
var import_error_codes = require("@better-auth/core/utils/error-codes");
var ERROR_CODES = (0, import_error_codes.defineErrorCodes)({
...
function createMiniAppEndpoints(config) {
  return {
    signInWithMiniApp: (0, import_api2.createAuthEndpoint)(
      ...
      async (ctx) => {
        ...
        if (!initData || typeof initData !== "string") {
          throw import_api2.APIError.from("BAD_REQUEST", ERROR_CODES.INIT_DATA_REQUIRED);
        }
        if (!config.botToken) {
          throw import_api2.APIError.from(
        ...
        if (!data.user) {
          throw import_api2.APIError.from("BAD_REQUEST", ERROR_CODES.NO_USER_IN_INIT_DATA);
        }
        ...
        await (0, import_cookies.setSessionCookie)(ctx, {
          session,
          user
        });
        return ctx.json({
    ...
    validateMiniApp: (0, import_api2.createAuthEndpoint)(
...
// src/oidc.ts
var import_oauth2 = require("@better-auth/core/oauth2");
...
var getTelegramPublicKey = async (kid, algorithm, timeout) => {
  const { data } = await (0, import_fetch.betterFetch)(TELEGRAM_OIDC_JWKS_URI, { timeout, retry: 0 });
  if (!data?.keys) {
  ...
  return await (0, import_jose.importJWK)(jwk, algorithm);
};
...
function createTelegramOIDCProvider(botToken, options = {}) {
  ...
  const verifyToken = async (token, nonce) => {
    try {
      const { kid, alg } = (0, import_jose.decodeProtectedHeader)(token);
      if (!(kid && alg)) {
        return null;
      }
      if (!clientId) {
      ...
      const { payload } = await (0, import_jose.jwtVerify)(token, publicKey, {
        algorithms: [alg],
  ...
  return {
    ...
    createAuthorizationURL({ state, codeVerifier, scopes, redirectURI }) {
      ...
      return (0, import_oauth2.createAuthorizationURL)({
        id: TELEGRAM_OIDC_PROVIDER_ID,
    ...
    validateAuthorizationCode({ code, codeVerifier, redirectURI }) {
      requireOIDCCredentials();
      return (0, import_oauth2.validateAuthorizationCode)({
        code,
        codeVerifier,
        redirectURI,
        options: providerOptions,
...
var import_cookies2 = require("better-auth/cookies");
function createWidgetEndpoints(config) {
  return {
    signInWithTelegram: (0, import_api3.createAuthEndpoint)(
      "/telegram/signin",
      {
        method: "POST"
      },
      async (ctx) => {
        if (!config.botToken) {
          throw import_api3.APIError.from(
            "INTERNAL_SERVER_ERROR",
            ERROR_CODES.BOT_TOKEN_REQUIRED
          );
        }
        ...
        if (!validateTelegramAuthData(body)) {
          throw import_api3.APIError.from("BAD_REQUEST", ERROR_CODES.INVALID_AUTH_DATA);
        }
        ...
        await (0, import_cookies2.setSessionCookie)(ctx, {
          session,
          user
        });
        return ctx.json({
    ...
    linkTelegram: (0, import_api3.createAuthEndpoint)(
      "/telegram/link",
      {
        method: "POST",
      ...
      async (ctx) => {
        if (!config.allowUserToLink) {
          throw import_api3.APIError.from("FORBIDDEN", ERROR_CODES.LINKING_DISABLED);
        }
        const body = await ctx.body;
        const session = ctx.context.session;
        if (!session?.user?.id) {
          throw import_api3.APIError.from("UNAUTHORIZED", ERROR_CODES.NOT_AUTHENTICATED);
        }
        if (!config.botToken) {
          throw import_api3.APIError.from(
            "INTERNAL_SERVER_ERROR",
    ...
    unlinkTelegram: (0, import_api3.createAuthEndpoint)(
      "/telegram/unlink",
      {
        method: "POST",
        use: [import_api3.sessionMiddleware]
      ...
      async (ctx) => {
        ...
        if (!account) {
          throw import_api3.APIError.from("NOT_FOUND", ERROR_CODES.NOT_LINKED);
        }
